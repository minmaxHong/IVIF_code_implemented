...
class PatchEmbedding(nn.Module):
    def __init__(self, in_channels=3, patch_size=16, emb_size=768, img_size=224):
        ...
        self.cls_tokens = nn.Parameter(torch.randn(1, 1, emb_size)) # class token
        self.positions = nn.Parameter(torch.randn((img_size // patch_size) ** 2 + 1, emb_size)) # position embedding
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # print('queries, keys, values: ', queries.size(), keys.size(), values.size())
...
class ViT(nn.Sequential):
    def __init__(self, in_channels=3, patch_size=16, emb_size=768, img_size=224, depth=12, n_classses=1000, **kwargs):
        super(ViT, self).__init__(
            PatchEmbedding(in_channels, patch_size, emb_size, img_size),
            TransformerEncoder(depth, emb_size=emb_size, **kwargs),
            ClassficationHead(emb_size, n_classses)
        )
